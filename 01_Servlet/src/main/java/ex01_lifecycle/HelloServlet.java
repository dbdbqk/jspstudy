...
public class HelloServlet extends HttpServlet {
	...
	*/
	...
	/*
    	2. init()
    		1) 서블릿 환경 설정을 담당하다.
    		2) init() 호출 뒤 자동으로 service() 메소드 호출된다.	
    */
	...
	*/
	...
	/*
		4.doGet()
			...
			2) GET 방식의 요청의 예시
				(1) <a href="http://localhost:9090/01_servlet/HelloServlet">		-- 주소를 직접 조작
				(2) <form action="http://localhost:9090/01_servlet/HelloServlet">	-- method 생략하면 get방식
				(3) location.href='http://localhost:9090/01_servlet/HelloServlet'	-- 주소를 직접 조작
				(4) open('http://localhost:9090/01_servlet/HelloServlet', '', '')	-- 주소를 직접 조작
				(5) $.ajax({														-- type 생략하면 get방식
						type: 'GET'
	...
	*/
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 응답 정보를 가진 개체 : response
		
		// 클라이언트로 정보(텍스트)를 보내기 위한 출력 스트림 : response.getWriter() - PrintWriter
		
		// 출력 스트림으로 정보(텍스트)를 보내는 메소드 : append(), write(), print(), println()
		
		response.getWriter().append("Served at: ").append(request.getContextPath());	// Served at: /01_Servlet 보내겠다(서버 -> 클라이언트)
	}
