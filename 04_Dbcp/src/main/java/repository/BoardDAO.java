...
public class BoardDAO {
	...
	public List<BoardDTO> selectBoardList(){
		...
		try {
			...
			// 5.PreparedStatement 객체를 이용해 쿼리문 실행(SELECT문 실행은 executeQuery() 메소드가 한다.
			rs = ps.executeQuery();
	...
	public BoardDTO selectBoardByNo(int board_no) {
		...
		try {
			...
			// 6.PreparedStatement 객체를 이용해 쿼리문 실행(SELECT문 실행은 executeQuery() 메소드가 한다.)
			rs = ps.executeQuery();
	...
	public int insertBoard(BoardDTO board) {
		...
		try {
			...
			// 6.PreparedStatement 객체를 이용해 쿼리문 실행(INSERT문 실행은 executeUpdate() 메소드가 한다.
	...
	public int updateBoard(BoardDTO board) {
		...
		try {
			...
			// 6.PreparedStatement 객체를 이용해 쿼리문 실행(UPDATE문 실행은 executeUpdate() 메소드가 한다.
	...
	public int deleteBoard(int board_no) {
		...
		try {
			...
			// 5. 쿼리문에 변수 값 전달하기
			ps.setInt(1, board_no);	  // 1번째 물음표(?)에 title 전달하기
			
			// 6.PreparedStatement 객체를 이용해 쿼리문 실행(DELETE문 실행은 executeUpdate() 메소드가 한다.
